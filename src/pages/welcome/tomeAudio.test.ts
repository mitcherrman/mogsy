...
describe("the scribble's window", () => {
  it("covers the longest slot the sequence can ask it to scratch through", () => {
    // The pen's window IS the slot's arrival time. The ceiling has to sit clear
    // of the longest window the sequence can ask for, or the pen falls silent
    // under something still arriving. Held against the real chapters so
    // re-tuning the cadence cannot quietly reintroduce that.
