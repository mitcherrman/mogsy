...
import { ArrowLeft, Megaphone, GripVertical, Trophy, Shuffle, Zap } from "lucide-react";
      ...
      <DialogContent className="max-w-sm p-0 overflow-hidden border-border bg-card">
        <div className="p-5">
          <div className="flex items-center gap-3 mb-5">
            <div className="h-12 w-12 rounded-full bg-secondary flex items-center justify-center text-xl flex-shrink-0 overflow-hidden">
              {league.imageUrl ? (
            ...
            <div className="min-w-0">
              <h3 className="font-extrabold text-foreground text-lg truncate">{league.name}</h3>
              <p className="text-xs text-muted-foreground">
                {league.memberCount} {league.type === "preset" ? "items" : "players"}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            <motion.button
              onClick={handleSwipe}
              className="relative rounded-2xl border border-border bg-background p-5 text-left transition-all hover:border-primary/40 hover:shadow-[0_0_30px_hsl(var(--primary)/0.12)] overflow-hidden group"
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <h2 className="text-lg font-extrabold text-foreground">Swipe</h2>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Vote head-to-head and move the rankings
                  </p>
                </div>
              </div>
            </motion.button>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <h2 className="text-lg font-extrabold text-foreground">Elo Check</h2>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Guess who's ranked higher — test your knowledge
                  </p>
                </div>
              </div>
            </motion.button>
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className={`flex flex-col items-center gap-2 rounded-2xl border bg-card p-4 transition-all duration-200 hover:border-primary/30 hover:shadow-[0_0_20px_hsl(var(--primary)/0.12)] h-full w-full ${
...
/* ─── Reorder wrapper ─── */
function DraggableLeagueCard({ league, reorderMode, onSelect }: { league: LeagueOption; reorderMode: boolean; onSelect: (l: LeagueOption) => void }) {
  const controls = useDragControls();

  if (!reorderMode) {
    return <LeagueCard league={league} reorderMode={false} onSelect={onSelect} />;
  }
  ...
  return (
    ...
    >
      <LeagueCard league={league} reorderMode controls={controls} onSelect={onSelect} />
    </Reorder.Item>
  );
}
...
function DraggableCategoryCard({
  ...
  const inner = (
    <div className="relative">
      ...
      <button
        onClick={onToggle}
        className={`flex flex-col items-center gap-2 rounded-2xl border bg-card p-4 transition-all duration-200 hover:border-primary/30 hover:shadow-[0_0_20px_hsl(var(--primary)/0.12)] w-full h-full ${
...
export default function Play() {
  ...
  const renderGrid = (leagues: LeagueOption[], onReorder: (v: LeagueOption[]) => void) => {
    if (reorderMode) {
      return (
        <Reorder.Group
          axis="y"
        ...
        >
          {leagues.map((league) => (
            <DraggableLeagueCard key={league.id} league={league} reorderMode onSelect={handleSelectLeague} />
          ))}
        </Reorder.Group>
      );
    }
    return (
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
        {leagues.map((league) => (
          <LeagueCard key={league.id} league={league} reorderMode={false} onSelect={handleSelectLeague} />
        ))}
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-8">
        ...
        <Tabs defaultValue="presets" className="w-full">
          ...
          <TabsContent value="users">
            {userLeagues.length === 0 ? (
              ...
              renderGrid(userLeagues, (newOrder) => {
                ...
                <>
                  ...
                  {expandedCat && (
                    ...
                    >
                      ...
                      {renderGrid(expandedCat.leagues, (newOrder) => {
      ...
      {/* Game mode picker */}
      <GameModePicker league={selectedLeague} onClose={() => setSelectedLeague(null)} />
    </div>
  );
}
