 ...
 *
 * FACTUAL ACCURACY IS DERIVED, NOT READ.
 * `get_league_swipe_stats` computes `correct` / `incorrect` / `accuracy` /
 * `mostMissed` from `league_swipe_results.is_correct`, which the v2 vote RPC
 * writes NULL unconditionally — the column was a browser claim the database
 * trusted, and v2 stopped trusting it. Those four fields are therefore dead:
 * not "waiting for data", structurally empty forever. This page ignores them
 * and derives the same numbers from public vote aggregates judged by the
 * canonical backend verifier. See fetchFactualCommunityAccuracy.
 */
export default function LeagueSwipeStats() {
  ...
  const { data: factual } = useQuery({
    queryKey: ["league-swipe", "factual-accuracy"],
    queryFn: fetchFactualCommunityAccuracy,
    staleTime: 60_000,
  });
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <SEOHead
        title={`${META_REFLEX_NAME} Stats | Community Rankings & Accuracy | Mogzy`}
        description={`Community stats for ${META_REFLEX_NAME}: top-rated champions, knowledge accuracy, most contested matchups, and biggest blowouts.`}
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-[#c9a84c]">
            ...
            <span className="text-[10px] uppercase tracking-[0.25em] font-bold">{META_REFLEX_NAME}</span>
          </div>
        ...
        >
          <Zap className="h-4 w-4" /> Play {META_REFLEX_NAME}
        </Link>
      </div>
      ...
      {stats && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
            ...
            {/* Derived from vote aggregates + the canonical verifier, NOT from
                stats.totals.accuracy (which reads the always-NULL is_correct). */}
            <StatTile
              label="Accuracy"
              value={factual?.accuracy != null ? `${factual.accuracy}%` : "—"}
              Icon={Check}
            />
            <StatTile
              label="Avg response"
                ...
                >
                  <div>
                    ...
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                      {g.mode}
                      {g.mode === "knowledge" && factual?.perGame[g.slug] != null && (
                        <> · {factual.perGame[g.slug]}% community accuracy</>
                      )}
                    </div>
                  </div>
          ...
          <div className="rounded-2xl border border-border bg-gradient-to-br from-[#0a1428]/90 to-[#0a0a1a]/90 p-4">
            ...
            {!factual || factual.mostMissed.length === 0 ? (
              ...
              <>
                ...
                {/* A capped aggregate that does not say so reads as a total. */}
                {factual.truncated && (
                  <p className="mt-3 text-[11px] italic text-muted-foreground">
                    Across the {factual.judgedPairs.toLocaleString()} most-played matchups.
                  </p>
                )}
              </>
            )}
                  ...
                  <ul className="space-y-2">
                    {myResults.map((r, i) => (
                      <li key={i} className="flex items-center gap-2 text-sm">
                        {/* Three states, not two. `verifiedCorrect === null` means
                            the answer could NOT be judged (entity retired, values
                            converged, or no evaluator for this stat) — showing a
                            red X there would tell a player they were wrong because
                            the data moved. It gets a neutral dash instead. */}
                        {r.verifiedCorrect === true ? (
                          <Check className="h-4 w-4 shrink-0 text-emerald-400" />
                        ) : r.verifiedCorrect === false ? (
                          <X className="h-4 w-4 shrink-0 text-[#ff4655]" />
                        ) : (
                          <span
                            title="Couldn't be checked against current data"
                            aria-label="Not checked"
                            data-testid="swipe-result-unjudged"
                        ...
                        <span className="flex-1 truncate text-foreground">
                          {r.selectedEntity} <span className="text-muted-foreground">vs {r.otherEntity}</span>
                        </span>
