...
describe("RIV1 — the item-primary hero is floored, not bare cqmin", () => {
  ...
  it("sizes the hero icon off the band-stepped item token", () => {
    ...
    const icon = container.querySelector("[data-subject-hero-icon]") as HTMLElement;
    // RIV1 floored this at `max(14cqmin,calc(5.5*var(--sc-fit)))` — 88px in
    // the Ranked band. RIV2 moved the number into `--subject-hero-icon`, which
    ...
    expect(icon.className).toContain("h-[var(--subject-hero-icon)]");
    expect(icon.className).toContain("w-[var(--subject-hero-icon)]");
  });

  it("hangs the container-query scale hook on both token hosts", () => {
    // `--subject-hero-icon` is declared on `[data-subject-media]`. The hero zone and
  ...
  it("keeps the hero icon square, so item art is never stretched", () => {
    const { container } = render_(itemCostQuestion());
    const cls = (container.querySelector("[data-subject-hero-icon]") as HTMLElement).className;
  ...
  it("derives the whole composition from the item, so the ratios cannot drift", () => {
    ...
    // decorative layer is a MULTIPLE of `--subject-hero-icon` (or of
    // `--subject-medallion`, which is itself one), so enlarging the item enlarges
    ...
    for (const token of [
      "var(--subject-medallion)",
      "var(--subject-medallion-inner)",
      "calc(1.05*var(--subject-hero-icon))",   // glow
      "calc(0.68*var(--subject-hero-icon))",   // pedestal offset
    ]) {
...
describe("RIV2 — the item-primary card composes the approved shop scene", () => {
  ...
  it("drives the background echo from the SAME dynamic item art", () => {
    ...
    expect(echo!.className).toContain("h-[var(--subject-echo)]");
  });

  it("marks exactly one image as the foreground focal item", () => {
    const { container } = render_(itemCostQuestion());
    const focal = container.querySelectorAll("[data-subject-hero-icon]");
  ...
  it("leaves the RECIPE path out of the shop scene entirely", () => {
    ...
    expect(container.querySelector("[data-subject-media]")).toBeNull();
...
describe("RIV1 — nothing else inherited the item treatment", () => {
  it("leaves the collectible fallback on its own floor", () => {
    ...
    expect(container.innerHTML).not.toContain("--subject-hero-icon");
