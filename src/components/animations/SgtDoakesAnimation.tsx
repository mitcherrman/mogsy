...
/** Sgt Doakes walks in over the loser's card — "Surprise, motherfucker" */
export default function SgtDoakesAnimation({ winnerSide, items, onComplete }: Props) {
  const [phase, setPhase] = useState<"idle" | "appear" | "surprise" | "done">("idle");
  ...
  useEffect(() => {
    ...
    setPhase("appear");
    const t1 = setTimeout(() => setPhase("surprise"), 300);
                ...
                <motion.div key={idx} className="flex-1 flex flex-col min-h-0 rounded-2xl border border-border bg-card overflow-hidden relative"
                  animate={phase !== "idle" ? {
                    boxShadow: [
                      ...
                      "0 0 16px 4px hsla(0, 100%, 50%, 0.5)",
                      "0 0 16px 4px hsla(230, 100%, 50%, 0.5)",
                      "0 0 16px 4px hsla(0, 100%, 50%, 0.4)",
                      "0 0 16px 4px hsla(230, 100%, 50%, 0.4)",
                      "0 0 6px 2px hsla(230, 100%, 50%, 0.15)",
                    ],
                  } : {}}
              ...
              <div key={idx} className="flex-1 flex flex-col min-h-0 relative rounded-2xl border border-border bg-card overflow-hidden">
                <div className="w-full portrait:aspect-[5/4] landscape:aspect-[3/4] md:aspect-[3/4] relative overflow-hidden">
                  ...
                  <motion.div
                    ...
                    animate={
                      phase === "appear" || phase === "surprise" || phase === "done"
                        ? { opacity: 0.85, y: 0 }
                        : {}
                    }
                  ...
                  {(phase === "surprise" || phase === "done") && (
                    <motion.div
                      className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none"
                      initial={{ scale: 3, opacity: 0, rotate: -6 }}
                      animate={{ scale: 1, opacity: 1, rotate: -6 }}
                      transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }}
                    >
                      <span
                        className="text-3xl md:text-4xl lg:text-5xl font-black tracking-[0.12em] uppercase select-none"
                        style={{
                          color: "hsl(0, 70%, 55%)",
