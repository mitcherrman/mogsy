...
> Pure contract code, INTEGRATED and PUSHED (`origin/master` `6073e035`); wired into nothing**) and
> [`docs/gr1-reusable-state-window-composition.md`](./gr1-reusable-state-window-composition.md)
...
> itself is unchanged and remains the final authority**) and
> [`docs/gr1-champion-state-sequence-source.md`](./gr1-champion-state-sequence-source.md)
> (GR1 **the first production Champion `StateSequenceSource`, `champion.progression.legal_advance.v1`
> — integrated `e7436166`, wired into nothing**) and
> [`docs/gr1-champion-slice-profiles.md`](./gr1-champion-slice-profiles.md)
> (GR1 **Champion Slice Profiles — scope breadth vs question budget; five certified profiles
> (snapshot / tight / early_phase / wide / full_range_sample), checkpoint/spread state preference,
> a third Lab composition policy. Backend `c2634d8a` PUSHED to `origin/master`; Generator Lab only**) and
...
> preference as a second certified Lab policy beside an unchanged v1. Backend committed, NOT
> pushed; Generator Lab only, not wired to Ranked**).
...
| **GR1 Champion Slice composition v2** | **CERTIFIED FOR THE LAB, COMMITTED, NOT PUSHED, 2026-09-22. Generator Lab only; not the default anywhere; no Ranked wiring.** Backend **`3f18a75f`** on `gr1/slice-composition-v2` (worktree `~/lcs-wt-gr1-v2`), base `origin/master` `b5c37f48`; comparison base `~/lcs-wt-gr1-v2-base`. Docs on `gr1/slice-composition-v2-docs` (worktree `~/mogsy-wt-gr1-v2`), base `origin/main` `45a322d4`. `composition.lab_profile_gate_aware.v2` = v1's walk re-ranked by the experiment's `balanced` criteria (ONE shared implementation) before the SAME gate-aware search. v1 byte-identical (13,840 roster records, 0 mismatches). Feasibility v2 == v1 in every cell and against exhaustive enumeration. Multi-state repeated metric 34.9% → 0.1%. See [`gr1-champion-slice-composition-v2.md`](./gr1-champion-slice-composition-v2.md). |
...
## Reusable state — WINDOW COMPOSITION implemented, Generator Lab only (2026-09-20)

Full record: [`gr1-reusable-state-window-composition.md`](./gr1-reusable-state-window-composition.md).
Backend **`aa9e2ea5`** on `origin/master` (authored `93f3e28f` on `dd97112b`, rebased onto `21cf0c71`), worktree
`~/lcs-wt-gr1-compose`; comparison base `~/lcs-wt-gr1-compose-base` @ `dd97112b`. Docs base
`origin/main` **`1b4f60ea`**, worktree `~/mogsy-wt-gr1-compose`. **BOTH PUSHED.** No frontend
commit.

* **What it is.** The first connection of the pieces the previous phases built: `StateWindow →
  resolve_window_states → the EXISTING Phase 3 state-aware cooldown candidates per resolved state →
  ONE deduplicated multi-state candidate universe → ONE composed question set → a MULTI-STATE
  `FrozenStateBundle`. **6 files, all inside `mastery/setup_state/` or `mastery/tests/`** (3 new, 3
  modified). `composition.py` is in the isolation guard's **CONTRACT** half — so a composer provably
  cannot reach a database, carries no game-rule number and holds no family id, roster or metric name
  of its own — and `window_lab.py` is the CONSUMER that connects the two halves.
* **No identity rule was added.** Collapse is keyed on `candidate_id()`, which is already the
  repository's semantic question identity, and provenance is kept BESIDE it (`source_ordinals`),
  never inside it. Two builds resolving to 20 haste give ONE question produced by two states (whose
  `semantic_state_key`s genuinely differ — asserted); 20 vs 40 haste stay two; a rank change stays
  two; and **Ahri Q r3 → r4 → r4 collapses to one, because Ahri's Q cooldown is FLAT and a
  rank-invariant cooldown is asked without a rank.** One identity carrying two `content_digest`s is
  **refused** (`candidate_answer_conflict`) rather than won by whichever was read last.
* **Every window node is accounted for.** A state that resolves and has nothing askable is a recorded
  `BarrenState` carrying the family's OWN refusal code; producers ∪ barren is the whole window and
  the two are disjoint, asserted. A state that cannot be RESOLVED still fails the whole window,
  naming its node — there is no partial window. A window with nothing askable anywhere is refused
  whole.
* **The composition policy is EXPERIMENTAL and says so.** `composition.lab_window_coverage.v1`:
  group by presentation state (the earliest producer), spread each state's near-identical variants
  apart by bucketing on `subject_ref` and interleaving, then round-robin across the window from a
  seeded offset. Deterministic from `(seed, budget, universe)` alone — no clock, no RNG. **The
  budget is a plain count independent of the number of states**, no state contributes more than
  `ceil(budget/groups)`, there are no duplicates, and a budget larger than the supply returns the
  whole universe with **nothing invented to fill it**. It is not the Slice policy; the current Slice
  composer and `_pattern_group` are byte-identical.
* **Ordering: progression first.** `(primary_ordinal, family_id, candidate_key)`. **The seed chooses
  WHICH questions are asked and never reorders them** — nothing is globally randomized after
  composition, because a set whose states interleave unpredictably is not a progression a reader can
  follow.
* **The first multi-state frozen bundle, and it needed no type change.** Only the states a selected
  question needs are included, in window order, `state_index` remapped densely from zero
  deterministically; `step_index` is the composed presentation order; each step keeps its
  `ScenarioBinding`, `answer_metric`, `candidate_id` and `content_digest`; `derived_used` is narrowed
  to what the selected questions read; display labels are merged and a conflicting label refused. It
  round-trips exactly through `json` and through the Phase 4B seam into a real sqlite `TEXT` column,
  and **verifies with `sqlite3.connect` made to raise**. Nothing is persisted.
* **`FrozenTransition` is populated only where it can be honest.** Contiguous included states and one
  side: then `(0→1)`, `(1→2)` with changed axes and a delta. Non-contiguous or multi-side: **empty,
  with the reason reported** — a partial tuple cannot be told from an absent change, and a
  transition across a gap would read as one step of a progression while describing several.
* **Recorded finding — composition and publication are two policies and they disagree.** A composed
  set of three questions about one ability at three ranks is **refused by the production gate**:
  `PublicationBlocked: policy gating left the recipe unsatisfied (request 'recall_ability_cooldown'
  asked for 3, only 2 available)`. That refusal is correct, is not routed around, is asserted by a
  test, and is why rendering through the production path is **optional** here. **A production Slice
  policy that ignores the gate's diversity rule will compose sets that cannot be served** — the most
  useful thing this phase learned, and now the first blocker.
* **Generator Lab: backend JSON only, no endpoint.** `window_lab.window_diagnostic()` shows the
  window, each state with its candidates/skips/barren reason, the deduplicated universe with
  per-candidate source states, the composed set with its coverage, the questions in progression
  order and the verified bundle with its dense index map. `literal_window()` is the only sequence
  builder and orders exactly what it is handed. The Lab route set is pinned by exact set equality
  and was not widened; frontend untouched.
* **Nothing moved.** Champion Mastery, Matchup Mastery, Ranked, the current Slice, `_pattern_group`,
  `mastery_slice.py`, `lab.py`, `scenario.py`, the Lab route, `persistence.py` and
  `mastery/serving/state.py` are all byte-identical. `default_sequence_registry()` is still EMPTY
  (asserted), `rule.haste_ladder.v1` is unregistered, `champion_item_builds.json` is not consumed,
  no `mastery_state` is written, no DDL, no migration, no new family, no Full. A test scans the
  tracked file list for any production importer of `mastery.setup_state.window_lab` and finds none;
  the one-file isolation allow-list is unchanged.
* **Tests.** `mastery/tests` failure **SET** byte-identical to base: 5 failed both arms, 2266 →
  **2337** passed pre-commit and **2344** passed / 7 skipped post-commit; the +71 reconciles exactly
  as 63 new tests plus 8 new parametrised isolation cases, and the 14 → 7 skip fall is the
  committed-footprint guards running, as every earlier phase recorded. Focused setup_state arm
  **431 passed**. Ranked/Mastery arm **2 failed, 213 passed**, both `test_mastery_ranked_capsule`'s
  pinned capsule ids and digests, **re-run on a clean detached worktree at `dd97112b` and identical
  there**.
* **Next-phase boundary.** Three things, and nothing here assumes which comes first: **a real state
  source** (until one exists, every window and composition policy is fitted to a fixture);
  **persisting a multi-state bundle on a served segment** (one line beside the `served_artifact.build`
  `generate_segment` already calls, and it must not happen before something can produce a
  state-aware segment); and **reconciling the composition policy with the publication gate**, which
  this phase proved is not optional.
...
## Reusable state — CHAMPION SLICE PROFILES, Generator Lab only (2026-09-21)

Full record: [`gr1-champion-slice-profiles.md`](./gr1-champion-slice-profiles.md). Backend
**`c2634d8a`** on `gr1/champion-slice-profiles`, worktree `~/lcs-wt-gr1-prof`, parent
`origin/master` **`d5fbacd6`** (the brief's `e7436166` moved once mid-phase — Immolate target
multipliers, zero `mastery/` overlap — and the branch was rebased before committing). Comparison base
`~/lcs-wt-gr1-base` detached @ `d5fbacd6`. Docs on `gr1/champion-slice-profiles-docs`, worktree
`~/mogsy-wt-gr1-prof`, base `origin/main` **`4cf0b247`** (the brief's `884e96a5` had moved three
commits). **NEITHER PUSHED.** No frontend commit.

* **What it is.** The owner's direction that a Slice must not mean one fixed window size, made a
  contract. **Scope breadth** (which states are eligible) and **question budget** (how many
  questions) are independent inputs of a `ChampionSliceProfile`; the budget is in `profile_key` and
  never in `scope_key`, so budgets 3 and 4 over one profile select the identical window, eligible
  ordinals and state preference (asserted for every profile).

* **Five certified profiles** (`slice_profile.certified_profile(name, question_budget=…)`):
  `snapshot` (one exact state, several questions — first-class), `tight` (3-state local span around
  a seeded or requested anchor, dense/local), `early_phase` (level band 1–6), `wide` (level band
  1–11), `full_range_sample` (the whole run the source supplied). The three broad ones sample with
  **checkpoint/spread**. None is a Ranked preset; `parse_mastery_slice_config` refuses every profile
  name.

* **The audit finding that shaped it.** `StateWindow` already separated states from questions and
  is contiguous by contract, so every profile scope IS a window and "window" kept its meaning; a
  `SliceScope` sits above it. What was missing was spread: the coverage walk starts at a seeded
  offset in window order, so a broad window with a small budget clustered on adjacent states. The
  new `composition.lab_profile_gate_aware.v1` walks the **existing** gate-aware search (extracted,
  shared, behaviourally unchanged for both old policies) over the profile's STATE preference, and a
  shared question may now be **presented at any state that produces it** (`presentation_ordinal`,
  default `None` = the old earliest-producer rule; refused if not a producer). Without that, a spread
  profile could never represent a later state whose questions all first appeared earlier.

* **Scope is sequence-based.** Full range = whatever run the source supplied (monkeypatching the
  rules to grant more levels grows it, no code edit). A band is a request checked against the nodes'
  own values: a run missing a bound, a non-contiguous band, or an axis the run does not state is
  **refused**, never clamped. The certified requests (3, 6, 11) live in ONE named assignment,
  `_CERTIFIED_SCOPE_REQUESTS`, and the contract-half number guard was deliberately given exactly one
  named exemption for it (mirroring its existing slot-letter exemption) rather than routed around.

* **Checkpoints are read off the sequence's derived transitions** — a non-routine axis change, a key
  activated (a slot's first point, an item appearing), a key removed (components consumed). A pure
  level run has no signal anywhere and falls back to spread + seed; nothing reads "level 6" as an
  ultimate.

* **Results (level source; "ranks" = a stated, attributed harness skill order).** Ahri full range b=3
  → levels 2/10/18, b=4 → 2/6/10/18; Jarvan IV feasible on 8 of 10 profile×budget cells (its seed-`alpha` snapshot state holds 2); **Teemo**
  (flat) has 2 semantic questions across 18 states and every profile underfills with
  `semantic_supply_below_budget`; **Karma** (fail-closed rank run) takes every profile in LEVEL mode
  and reports the family's own `required_axis_absent`. Roster (164 rank-bearing): full range b=3
  **162/164**, early phase b=3 146, snapshot b=4 only **28** — the cooldown family's ≤1 question per
  learned ability, not a profile defect. Seed variation is real on Full Range (13–15 distinct
  question sets over 24 seeds for Jarvan) and deliberately concentrated on Early Phase / Wide by
  checkpoint priority.

* **Structured infeasibility.** `run_profile` returns `infeasibility` with the gate's own
  `limiting_codes`, a separate `supply_codes` (`semantic_supply_below_budget` — the preflight
  synthesises its recipe from the selection and cannot phrase it; the gate was not touched),
  `requested_budget` and `feasible_count`. No filler, no repeat, no silent shrink; `compose_profile`
  raises instead. A stubbed yes-oracle still cannot get the monotonous triple past `publish`.

* **Tests.** New `test_gr1_champion_slice_profiles.py`: **93 passed**. Isolation guard 102 → 108.
  `mastery/tests`: base 5 failed / 2457 passed / 14 skipped, branch 5 failed / 2563 passed / 7
  skipped — failure SET identical by name, counts reconcile exactly. Ranked/Mastery integration arm
  identical on both (2 pre-existing pinned-capsule failures).

* **Recorded, not built: State Context UI** — hover (desktop) / tap (mobile) context icons for
  champion, level, ability, items, runes/shards; an item such as Malignance opens a compact canonical
  item preview from the same authority as the question. No frontend code.

* **Deliberately unwired:** no Ranked, no route, no `mastery_slice` mode, no persistence, no
  `mastery_state` writer, no migration, no new family, no Full, no Matchup profile, cooldown prompt
  untouched, `progression.py` byte-identical, contract-half `default_sequence_registry()` empty.

* **Rollback:** `git revert c2634d8a`.

## Reusable state — STATE-AWARE CHAMPION STATS, Generator Lab only (2026-09-21)

Full record: [`gr1-state-aware-champion-stats.md`](./gr1-state-aware-champion-stats.md). Backend
**`81e5b24d`** on `gr1/state-aware-champion-stats`, worktree `~/lcs-wt-gr1-stats`, authored on
`origin/master` `c2634d8a` and rebased onto **`a178a116`** (one items-only commit, zero `mastery/`
overlap). Comparison base `~/lcs-wt-gr1-stats-base` detached @ `c2634d8a`. Docs on
`gr1/state-aware-champion-stats-docs`, worktree `~/mogsy-wt-gr1-stats`, base `origin/main`
`7d003128`. **Integrated 2026-09-21:** backend rebased onto `71919f50` and pushed as
**`1090633f`**; docs pushed to `origin/main`. No frontend commit.

* **What it is.** The EXISTING `champion_stat_level` family made the second consumer of
  `ResolvedState` — not a new family. Same family id; candidates built by the bank's own
  constructor (extracted unchanged into public `bank.level_stat_candidate`), so a state at level 11
  asks the bank's level-11 question with the same `candidate_id` / `content_digest`. The answer is
  the state's `<stat>.at_level` (Phase 2 derived it with the bank's exact call); disagreement with the
  projected fact is refused. No formula in the new module (AST-guarded).
* **Supported metrics** (level 9, 173 champions): health 173, AD 172, armor 172, MR 173, health
  regen 172, mana 145, mana regen 145. Refused with the existing family's own reasons: level 1
  (`level_redundant_with_base`), movement speed / attack range (`stat_not_level_scaled`), zero growth
  (Senna AD, Thresh armor, Briar hp5), mana questions without a mana pool (28 — including energy,
  whose `mp5` column holds energy regen), attack speed (no certified level fact), a stated form
  (Mega Gnar would read Mini Gnar's row), hidden dependencies. Intrinsic only — no item `.total`.
* **Identity.** No binding: champion + metric + level. Items/ranks/provenance never move it; a level
  change always does. Stat questions never collapse across a level run (they change every level) and
  do collapse across equivalent states (`source_ordinals` kept).
* **One universe.** `families.py` table + opt-in `families=` on the Lab path (default cooldown-only,
  byte-identical). Families pooled per state through the same gate filters; a family refusing a
  state no longer makes it barren; `derived_used` narrows per family (a stat-only state never freezes
  ability haste). The composer's one change is a generic per-candidate `family_id` map.
* **Impact (seed `alpha`).** Rank arm, 164 champions, before → after: Snapshot b=3 92 → **157**,
  b=4 **28 → 157**; Tight 138/116 → **164/164**; Early Phase 146/130 → **164/164**; Wide 155/146 →
  **164/164**; Full Range 162/159 → **164/164**. LEVEL source, 173: every profile 0 → **173** except
  Snapshot 163 (the 10 misses all anchored on level 1). All 9 rank-gap champions now fill every
  profile at 3 and 4 in LEVEL mode (their rank rules untouched).
* **Invariance.** 173 Mastery banks, 1,670 Practice questions, 78 default Lab diagnostics: 22.8 MB
  dumps `cmp`-identical vs `c2634d8a`. `mastery/tests` 5/2556/14 → 5/2648/7, failure SET identical;
  integration arm 2/331 on both.
* **Surfaced, not fixed:** the Mastery bank itself asks "base mana regen" for the 28 champions
  without a mana pool (it gates `base_mana` but not `base_mana_regen`). Level 1 as a question and
  metric diversity inside a set are owner/composition decisions.
* **Integration recheck:** still unfixed and deliberately so — the bank also carries `mp5:lvl11`/`lvl18`
...
none started: **(a)** a Lab-only profile-distribution experiment (profile × seed × roster, measuring
variety); **(b)** owner decisions — level 1 as a stat question, metric diversity inside a set, the
prompt-rank question; **(c)** a third family (ability cost at rank, or item-modified totals with a
binding); **(d)** closing the 9 rank-bearing gaps upstream (now only blocking rank-state questions).
Also queued, separately: the Mastery bank's mana-regen question for 28 manaless champions.

**Superseded — pick ONE of the Slice-profile phase's next options, and do NOT wire Ranked.**
Champion Slice Profiles now exist (see the row above and
[`gr1-champion-slice-profiles.md`](./gr1-champion-slice-profiles.md) §19), which closes item 1 of
the superseded block below: there is now a product-shaped window/scope policy, in the Lab. The
candidates, none started: **(1)** a second state-aware family (stats / resource cost / ratio) — the
biggest lever on Snapshot and Tight and the real test that profiles are not cooldown-shaped;
**(2)** the owner's prompt-rank content decision; **(3)** a Lab-only profile-distribution experiment
(profile × seed × roster) to inform which profiles Ranked could carry; **(4)** persisting a
multi-state bundle on a served segment — still only once something can produce a state-aware segment
for players; **(5)** closing the 9 rank-bearing gaps upstream. Still open for the owner: which
profiles enter Ranked and with what weights, how strongly checkpoints should concentrate vs seed
replayability, whether an unfillable budget refuses / re-draws / ships fewer, Snapshot's tier.

**Superseded — a PRODUCT window policy, or persisting a multi-state bundle on a served
