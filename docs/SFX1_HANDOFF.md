...
### SFX1.8 — final QA and mix certification
...
Implement **SFX1.8 only — final QA and mix certification**: run and document a whole-site semantic-sound matrix across landing, Swipe/Elo/card animations, Shop, Welcome, Home Hub, Leaguecraft, Ranked, Combat, Archives, Pro Play, auth/account, and Quiz Broadcast. Certify relative mix, global mute/unmute and cross-tab changes, first-gesture/autoplay recovery, background/foreground and route lifecycle, missing/offline asset fail-soft behavior, Audio Studio overrides, and coexistence with Academy Radio and Mode Soundtrack. Run the full relevant unit/integration suite, TypeScript, production build, targeted lint, and production-config smoke tests; perform desktop Chrome/Edge, Firefox, and Safari plus iOS Safari and Android Chrome checks on speakers and headphones where hardware is available. Fix only verified SFX defects, add no new semantic cues or product behavior, and leave music architecture, gameplay/data authority, routes, backend, and unrelated worktree changes untouched.
