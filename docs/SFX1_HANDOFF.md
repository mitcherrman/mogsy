...
| Quiz Broadcast SFX | `lib/quiz-broadcast/sfx.ts`, `BroadcastSfxLayer.tsx`, `SfxSettings.tsx` | Broadcast-only question/reveal/tick/transition audio | Broadcast session/config, default off | A new `Audio` per event + unlock context | Broadcast renderer | Current specialist subsystem; keep its session policy, but reuse canonical renderer later |
...
| Broadcast `questionStart`, `countdownTick`, `reveal`, `correctAnswer`, `transition` | Broadcast phase/timers | Configured samples; repository only bundles `reveal.mp3` | Broadcast config, default off/empty | Broadcast-only switch, not visitor mute | Reveal phase requests both `reveal` and `correctAnswer`, so two configured files stack |

Configured but unreachable: `bubble_tap` has no runtime consumer; UI `success` and UI `error` have no call sites. `public/audio/sfx/` contains only a README.
...
- `bubble_tap` is admin/config residue with no runtime call. UI `success`/`error` are reserved but unused.
...
### SFX1.7 — broadcast renderer convergence and cleanup
