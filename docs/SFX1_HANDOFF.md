...
## Production integration — 2026-09-20

- Reconciled the approved eight-commit SFX1 result onto production baseline `fe0804c383afa6120d91cb565f590f14cd4f23d4` (`origin/main` at integration start). All eight reported SFX commits were present locally. They form one ancestry path from base `ce82867628c02658eded2ab62668b3a057874dce`, with two unrelated mobile-Hub commits interleaved before SFX1.2 and SFX1.3; those unrelated commits were not replayed as part of this integration.
- Integration used the eight approved commit diffs, accumulated without intermediate commits in an isolated worktree. The staged tree contains exactly the union of files touched by those eight commits. The only textual conflicts were in `QuizRankedMatch.tsx` and the Ranked shell probe: canonical live-match SFX observation and the query-gated SFX QA fixture were added while retaining the newer RFX1 server-owned entry timing, presentation cutoff, media preparation, reduced-motion behavior, and cold-entry probe controls.
